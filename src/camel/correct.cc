...
//       [&reads_overlaps](biosoup::Overlap const& ovlp) ->
//       OverlapEdlibAlignment {
//     auto const [query_str, target_str] = OverlapStrings(reads_overlaps,
//     ovlp);
//
//     return {.ovlp = ovlp, .edlib_result = AlignStrings(query_str,
//     target_str)};
//   };
//
...
//
//           [](biosoup::Overlap const& ovlp, std::uint32_t const lhs_id) ->
//           bool {
...
//                    ++ovlp_iter) {
//                 events.push_back(((ovlp_iter->lhs_begin >> kShrinkShift) +
//                 1U)
//                                  << 1U);
//                 events.push_back(
//                     (((ovlp_iter->lhs_end >> kShrinkShift) - 1U) << 1U) |
//                     1U);
//               }
//             }
//           }
...
auto SnpErrorCorrect(
  ...
  // timer.Start();
  // auto const kEstimatedCovg = detail::EstimateCoverage(state,
  // reads_overlaps); fmt::print(stderr,
  //            "[camel::SnpErrorCorrect]({:12.3f}) estimated coverage: {}\n",
  //            timer.Stop(), kEstimatedCovg);

  auto const ovlps = FindConfidentOverlaps(state, map_cfg, src_reads);
